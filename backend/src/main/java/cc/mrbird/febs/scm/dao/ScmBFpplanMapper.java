...
public interface ScmBFpplanMapper extends BaseMapper<ScmBFpplan> {
    ...
    @Update("UPDATE scm_b_fpplan set IS_DELETEMARK=0 where ID=#{lid};UPDATE scm_b_supplyplan SET CODE = '',FPHM = '',fprq= null WHERE CODE=#{id}  and status=1 ")
    void updateDeletePlan(@Param("id") String id,@Param("lid") Long lid);
        /*
        已经收获都不可以更改
         */
        @Update("UPDATE scm_b_supplyplan SET CODE = '' WHERE CODE =#{ids} and status=1 ")
    ...
    @Update("UPDATE scm_b_supplyplan SET fprq = #{fprq},fphm=#{fphm} WHERE CODE =#{code} and status=1 ")
