...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...vScmBGyspicUser}) {
                let fields = [     'code'       ,  'name'       ,  'gysaccount'       ,  'materId'       ,  'matnr'       ,  'charge'           ,  'fileId'                 ,  'mtart'       ,  'mseht'       ,  'txz01'       ,  'produceArea'       ,  'spec'       ,  'spellCode'       ,  'AuditCause'       ,  'werks'       ,  'werkt'       ,  'lgort'       ,  'lgortName'    ]
                let fieldDates = [                                                                                                                  ]
                Object.keys(vScmBGyspicUser).forEach((key) => {
                    if (fields.indexOf(key) !== -1) {
                    this.form.getFieldDecorator(key)
                    let obj = {}
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        if(vScmBGyspicUser[key]!=='') {
                            obj[key] = moment(vScmBGyspicUser[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = vScmBGyspicUser[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.vScmBGyspicUser.id = vScmBGyspicUser.id
            },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let vScmBGyspicUser = this.form.getFieldsValue()
                        vScmBGyspicUser.id = this.vScmBGyspicUser.id
                    this.$put('vScmBGyspicUser', {
                        ...vScmBGyspicUser
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
