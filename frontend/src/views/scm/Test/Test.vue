<template>
  ...
  <template v-if="advanced">                                           <a-col :md="8" :sm="24">
        ...
        <div>
            <div class="operator">
                <a-button v-hasPermission="['test:add']" type="primary" ghost @click="add">新增</a-button>
                <a-button v-hasPermission="['test:delete']" @click="batchDelete">删除</a-button>
                <a-dropdown v-hasPermission="['test:export']">
                    <a-menu slot="overlay">
            ...
            <a-table ref="TableInfo"
                ...
                <template slot="operation" slot-scope="text, record">
                    <a-icon v-hasPermission="['test:update']" type="setting" theme="twoTone"
                            twoToneColor="#4a9ff5" @click="edit(record)" title="修改"></a-icon>
                    <a-badge v-hasNoPermission="['test:update']" status="warning" text="无权限"></a-badge>
                </template>
            </a-table>
        </div>
        <!-- 新增字典 -->
...
<script>
    ...
    onmodifyTimeToChange (date, dateString) {
        this.queryParams.modifyTimeTo = dateString
            ...
            edit(record) {
                this.$refs.testEdit.setFormValues(record)
            ...
            batchDelete() {
                ...
                this.$confirm({
                    ...
                    centered: true,
                    onOk() {
                        let testIds = that.selectedRowKeys.join(',')
                        that.$delete('test/' + testIds).then(() => {
