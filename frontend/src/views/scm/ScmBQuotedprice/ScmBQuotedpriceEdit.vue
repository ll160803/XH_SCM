...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...scmBQuotedprice}) {
                let fields = [  'id'     ,  'code'     ,  'baseId'     ,  'productName'     ,  'quotedPrice'     ,  'supplyAmount'     ,  'comments'     ,  'state'     ,  'displayIndex'     ,  'gysname'     ,  'gysaccount'     ,  'isDeletemark'     ,  'createTime'     ,  'modifyTime'     ,  'createUserId'     ,  'modifyUserId'   ]
                let fieldDates = [                                       'createTime'         , 'modifyTime'           ]
                Object.keys(scmBQuotedprice).forEach((key) => {
                    if (fields.indexOf(key) !== -1) {
                    this.form.getFieldDecorator(key)
                    let obj = {}
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        if(scmBQuotedprice[key]!=='') {
                            obj[key] = moment(scmBQuotedprice[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = scmBQuotedprice[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.scmBQuotedprice.id = scmBQuotedprice.id
            },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let scmBQuotedprice = this.form.getFieldsValue()
                        scmBQuotedprice.id = this.scmBQuotedprice.id
                    this.$put('scmBQuotedprice', {
                        ...scmBQuotedprice
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
