        ...
        methods: {
            ...
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    this.setFields()
                    this.$post('scmBQuerypriceProduct', {
                        ...this.scmBQuerypriceProduct
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
            ...
            setFields () {
                let values = this.form.getFieldsValue([  'id'     ,  'baseId'     ,  'productName'     ,  'isDeletemark'     ,  'createTime'     ,  'modifyTime'     ,  'createUserId'     ,  'modifyUserId'   ])
                if (typeof values !== 'undefined') {
                    Object.keys(values).forEach(_key => { this.scmBQuerypriceProduct[_key] = values[_key] })
                }
            }
        }
    }
