...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...scmDHrpmater}) {
                let fields = [  'id'     ,  'code'     ,  'name'     ,  'matnr'     ,  'mtart'     ,  'meins'     ,  'mseht'     ,  'maktx'     ,  'zeinr'     ,  'normt'     ,  'state'     ,  'bklas'     ,  'isDeletemark'     ,  'createTime'     ,  'modifyTime'     ,  'createUserId'     ,  'modifyUserId'   ]
                let fieldDates = [                                          'createTime'         , 'modifyTime'           ]
                Object.keys(scmDHrpmater).forEach((key) => {
                    ...
                    {
                        if(scmDHrpmater[key]!=='') {
                            obj[key] = moment(scmDHrpmater[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = scmDHrpmater[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.scmDHrpmater.id = scmDHrpmater.id
            },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let scmDHrpmater = this.form.getFieldsValue()
                        scmDHrpmater.id = this.scmDHrpmater.id
                    this.$put('scmDHrpmater', {
                        ...scmDHrpmater
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
