<template>
  ...
  >
          ...
          <a-row>
            ...
            >
              <a-form-item
                ...
                :wrapperCol="{span: 15, offset: 1}"
              >
                <a-input v-model="queryParams.gysaccount" />
              </a-form-item>
            </a-col>
            <a-col
              :md="6"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <a-select-option value="3">信息发布</a-select-option>
                  <a-select-option value="4">退回确认</a-select-option>
                  <a-select-option value="5">已开发票</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
          </a-row>
    ...
    <div>
      <div class="operator">
        <a-button
          v-hasPermission="['scmBSalereturn:add']"
          type="primary"
          ghost
          @click="add"
        >新增</a-button>
        <a-button
          v-hasPermission="['scmBSalereturn:delete']"
          @click="batchDelete"
        >删除</a-button>
        <a-button
          v-hasPermission="['scmBSalereturn:submit']"
          @click="batchSubmit"
        >提交</a-button>
        <a-button
          v-hasPermission="['scmBSalereturn:back']"
          @click="batchBack"
        >退回确认</a-button>
        <a-dropdown v-hasPermission="['scmBSalereturn:export']">
          <a-menu slot="overlay">
            <a-menu-item
              key="export-data"
              @click="exportExcel"
      ...
      >
        ...
        >
          <a-icon
            v-hasPermission="['scmBSalereturn:update']"
            type="setting"
            theme="twoTone"
            twoToneColor="#4a9ff5"
            @click="edit(record)"
          ...
          ></a-icon>
          <a-badge
            v-hasNoPermission="['scmBSalereturn:update']"
            status="warning"
            text="无权限"
          ></a-badge>
        </template>
...
<script>
import ScmBSalereturnAdd from './ScmBSalereturnAdd'
import ScmBSalereturnEdit from './ScmBSalereturnEdit'

export default {
  name: 'ScmBSalereturn',
  components: { ScmBSalereturnAdd, ScmBSalereturnEdit },
  data () {
    return {
      advanced: false,
      dataSource: [],
      ...
      if (flag == 0) {
        ...
        this.$confirm({
          ...
          onOk () {
            ...
            that.$delete('scmBSalereturn/submit/' + scmBSalereturnIds).then(() => {
    ...
    batchBack () {
      ...
      if (flag == 0) {
        ...
        this.$confirm({
          ...
          onOk () {
            let scmBSalereturnIds = that.selectedRowKeys.join(',')
            that.$delete('scmBSalereturn/back/' + scmBSalereturnIds).then(() => {
