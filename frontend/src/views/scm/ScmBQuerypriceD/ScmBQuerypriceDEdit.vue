...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFormValues ({...scmBQuerypriceD}) {
                let fields = [  'id'     ,  'baseId'     ,  'gysaccount'     ,  'gysname'     ,  'gyspym'     ,  'gyslabel'     ,  'commentsD'     ,  'isDeletemark'     ,  'createTime'     ,  'modifyTime'     ,  'createUserId'     ,  'modifyUserId'   ]
                let fieldDates = [                           'createTime'         , 'modifyTime'           ]
                Object.keys(scmBQuerypriceD).forEach((key) => {
                    if (fields.indexOf(key) !== -1) {
                    this.form.getFieldDecorator(key)
                    let obj = {}
                    if(fieldDates.indexOf(key)!==-1)
                    {
                        if(scmBQuerypriceD[key]!=='') {
                            obj[key] = moment(scmBQuerypriceD[key])
                        }
                        else
                        {
                            obj[key] =''
                        ...
                        obj[key] = scmBQuerypriceD[key]
                    }
                    this.form.setFieldsValue(obj)
                }
            })
                this.scmBQuerypriceD.id = scmBQuerypriceD.id
            },
            handleSubmit () {
                this.form.validateFields((err, values) => {
                    if (!err) {
                    let scmBQuerypriceD = this.form.getFieldsValue()
                        scmBQuerypriceD.id = this.scmBQuerypriceD.id
                    this.$put('scmBQuerypriceD', {
                        ...scmBQuerypriceD
                    }).then(() => {
                        this.reset()
                        this.$emit('success')
