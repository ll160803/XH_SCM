<template>
  ...
  >
    ...
    <div>
      <div class="operator">
        <a-button
          v-hasPermission="['scmBReport:add']"
          type="primary"
          ghost
          @click="add"
        >新增</a-button>
        <a-button
          v-hasPermission="['scmBReport:delete']"
          @click="batchDelete"
        >删除</a-button>
      </div>
      <!-- 表格区域 -->
      ...
      >
        ...
        >
          <a-icon
            v-hasPermission="['scmBReport:update']"
            type="setting"
            theme="twoTone"
            twoToneColor="#4a9ff5"
            @click="edit(record)"
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          let scmBReportIds = that.selectedRowKeys.join(',')
          that.$delete('scmBReport/' + scmBReportIds).then(() => {
