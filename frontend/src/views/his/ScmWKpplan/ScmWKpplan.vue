...
import PlanAdd from './ScmWKpplanAdd'
import PlanEdit from './ScmWKpplanEdit'
import { getLodop, getLodopDiv } from '../../../tools/lodop'
import FpplanPrint from './ScmWKpPrint.vue'
  ...
  methods: {
    ...
    subDelete (record, pRecord) {
      ...
      this.$confirm({
        ...
        centered: true,
        onOk () {
          that.$delete('scmWSale/deleteKpPlan/' + record.id).then((r) => {
            console.log(r)
            if (r.data == null) {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('scmWKpplan/' + scmBSendorderIds).then(() => {
            that.$message.success('删除成功')
