<template>
  ...
  >
    <a-form :form="form">
      ...
      <a-form-item v-bind="formItemLayout" label="库位">
        <a-input
          placeholder="请输入库位"
          v-decorator="[
            'kw',
      ...
      <a-form-item v-bind="formItemLayout" label="药品类别">
        <a-input
          placeholder="请输入药品类别"
          v-decorator="[
            'ypLb',
            { rules: [{ required: true, message: '药品类别不能为空' }] },
          ]"
        />
      </a-form-item>
      ...
      <a-form-item v-bind="formItemLayout" label="原进货价">
        <a-input
          placeholder="请输入原进货价"
          v-decorator="[
            'jhOld',
            { rules: [{ required: true, message: '原进货价不能为空' }] },
          ]"
        />
      </a-form-item>
      <a-form-item v-bind="formItemLayout" label="新进货价">
        <a-input
          placeholder="请输入新进货价"
          v-decorator="[
            'jhNew',
            { rules: [{ required: true, message: '新进货价不能为空' }] },
          ]"
        />
      </a-form-item>
      <a-form-item v-bind="formItemLayout" label="调价金额">
        <a-input
          placeholder="请输入调价金额"
          v-decorator="[
            'jhJe',
            { rules: [{ required: true, message: '调价金额不能为空' }] },
          ]"
        />
      </a-form-item>
    </a-form>
...
export default {
  ...
  methods: {
    ...
    setFormValues ({ ...scmWChange }) {
      let fields = ['tjRq', 'ny', 'fyId', 'kw', 'gysId', 'gysName', 'ypLb', 'ypBh', 'ypCd', 'ypMc', 'sl', 'dw', 'jhOld', 'jhNew', 'jhJe']
      let fieldDates = []
      Object.keys(scmWChange).forEach((key) => {
        if (fields.indexOf(key) !== -1) {
          this.form.getFieldDecorator(key)
          let obj = {}
          if (fieldDates.indexOf(key) !== -1) {
            if (scmWChange[key] !== '') {
              obj[key] = moment(scmWChange[key])
            }
            else {
              obj[key] = ''
            }
          } else {
            obj[key] = scmWChange[key]
          }
          this.form.setFieldsValue(obj)
        }
      })
      this.scmWChange.id = scmWChange.id
    },
    handleSubmit () {
      this.form.validateFields((err, values) => {
        if (!err) {
          let scmWChange = this.form.getFieldsValue()
          scmWChange.id = this.scmWChange.id                                        
          this.$put('scmWChange', {
            ...scmWChange
          }).then(() => {
            this.reset()
