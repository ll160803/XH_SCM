    ...
    <a-drawer
        ...
        <a-form :form="form">
                <a-form-item v-bind="formItemLayout"
                    label="1=草药代煎；2=核医学">
                        <a-input placeholder="请输入1=草药代煎；2=核医学" v-decorator="['ywlx', {rules: [{ required: true, message: '1=草药代煎；2=核医学不能为空' }] }]" />
                </a-form-item>
                <a-form-item v-bind="formItemLayout"
                    label="202210业务所属月份">
                ...
                <a-form-item v-bind="formItemLayout"
                    label="进货金额">
                        <a-input placeholder="请输入进货金额" v-decorator="['jhJe', {rules: [{ required: true, message: '进货金额不能为空' }] }]" />
                </a-form-item>
                <a-form-item v-bind="formItemLayout"
                    label="创建人姓名">
                        <a-input placeholder="请输入创建人姓名" v-decorator="['cjr', {rules: [{ required: true, message: '创建人姓名不能为空' }] }]" />
                </a-form-item>
                <a-form-item v-bind="formItemLayout"
                    label="创建人工号">
                        <a-input placeholder="请输入创建人工号" v-decorator="['cjrid', {rules: [{ required: true, message: '创建人工号不能为空' }] }]" />
                </a-form-item>
                <a-form-item v-bind="formItemLayout"
                    label="备注">
                        <a-input placeholder="请输入备注" v-decorator="['remark', {rules: [{ required: true, message: '备注不能为空' }] }]" />
                </a-form-item>
        </a-form>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            setFields () {
                let values = this.form.getFieldsValue([                    'ywlx'    ,                     'ny'    ,                     'fyId'    ,                     'gysId'    ,                     'gysName'    ,                     'ypBh'    ,                     'ypCd'    ,                     'ypMc'    ,                     'sl'    ,                     'dw'    ,                     'jhJe'    ,                     'cjr'    ,                     'cjrid'    ,                     'remark'                   ])
                if (typeof values !== 'undefined') {
                    Object.keys(values).forEach(_key => { this.scmWSale[_key] = values[_key] })
                }
            }
        }
    }
