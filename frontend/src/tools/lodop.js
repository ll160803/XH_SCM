...
export function getLodopDiv (obj) {
    lodopDiv = obj  
}
  ...
  try {
    ...
    if (needCLodop()) {
      ...
      if (!LODOP) {
        ...
        lodopDiv.innerHTML=strCLodopInstall + lodopDiv.innerHTML;
        return;
      } else {
        ...
        if (CLODOP.CVERSION < "3.0.0.2") {
          if (isIE) document.write(strCLodopUpdate);
            ...
            lodopDiv.innerHTML = strCLodopUpdate + lodopDiv.innerHTML;
        };
      ...
      if ((LODOP == null) || (typeof (LODOP.VERSION) == "undefined")) {
        if (navigator.userAgent.indexOf('Chrome') >= 0)
          lodopDiv.innerHTML = strHtmChrome + lodopDiv.innerHTML;
        if (navigator.userAgent.indexOf('Firefox') >= 0)
          lodopDiv.innerHTML = strHtmFireFox + lodopDiv.innerHTML;
        if (is64IE) document.write(strHtm64_Install);
        else
        if (isIE) document.write(strHtmInstall);
        else
          lodopDiv.innerHTML = strHtmInstall + lodopDiv.innerHTML;
        return LODOP;
      };
    };
    if (LODOP.VERSION < "6.0") {
      if (!needCLodop()) {
        ...
        if (isIE) document.write(strHtmUpdate);
          ...
          lodopDiv.innerHTML = strHtmUpdate + lodopDiv.innerHTML;
      };
      return LODOP;
    };
